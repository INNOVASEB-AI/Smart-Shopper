  ...
  try {
    ...
    if (productLink) {
      ...
      if (productLink.startsWith('/')) {
        productUrl = `https://www.checkers.co.za${productLink}`;
    ...
    if (!productUrl && productId) {
      productUrl = `https://www.checkers.co.za/p/${productId}`;
    }
    ...
    if (name && price && productId) {
      return {
        ...
        url: productUrl || `https://www.checkers.co.za/search-products/${encodeURIComponent(name)}`
      };
    }
  } catch (parseError) {
...
module.exports.scrapeCheckers = scrapeCheckers;
