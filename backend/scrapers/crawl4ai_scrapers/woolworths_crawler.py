...
This module implements a retailer-specific crawler for woolworths.co.za,
...
# Constants
WOOLWORTHS_SITEMAP_URL = "https://www.woolworths.co.za/sitemap.xml"

class WoolworthsCrawler(SimpleBaseCrawler):
    """
    Woolworths-specific crawler implementation
    """
    ...
    def __init__(
        ...
        """
        Initialize the Woolworths crawler
        ...
        """
        # Define a URL filter function for Woolworths
        def woolworths_url_filter(url: str) -> bool:
            """Filter for Woolworths product URLs"""
            return "/prod/" in url or "/product/" in url
        
        # Initialize the base crawler with more conservative settings for Woolworths
        super().__init__(
            retailer_name="Woolworths",
            sitemap_url=WOOLWORTHS_SITEMAP_URL,
            output_dir=output_dir,
            max_urls=max_urls,
            concurrency=concurrency,
            rate_limit=(2.0, 4.0),  # More conservative rate limits as specified in config
            url_filter=woolworths_url_filter
        )
        
        self.logger.info("Woolworths crawler initialized")
    
    async def process_result(self, result: Any, url: str) -> Optional[Dict[str, Any]]:
        """
        Process a Woolworths product page and extract data
        ...
        try:
            ...
            # Extract basic product info - adjust selectors for Woolworths
            product_data["title"] = await page.evaluate('''() => {
                const titleEl = document.querySelector("h1.product-title, .product-name, h1") || 
                               document.querySelector(".woolworths-product-title, [data-testid='product-title']");
            ...
            product_data["description"] = await page.evaluate('''() => {
                const descEl = document.querySelector(".product-description, .description") ||
                              document.querySelector(".woolworths-product-description, [data-testid='product-description']");
            ...
            price_text = await page.evaluate('''() => {
                const priceEl = document.querySelector(".price, .product-price, .current-price") ||
                               document.querySelector(".woolworths-price, [data-testid='price'], [data-testid='current-price']");
            ...
            product_data["brand"] = await page.evaluate('''() => {
                const brandEl = document.querySelector(".brand, .product-brand") ||
                               document.querySelector(".woolworths-brand, [data-testid='brand']");
            ...
            self.logger.error(f"Error processing Woolworths product at {url}: {str(e)}")
            return None

# Test function for the crawler
async def main():
    """Test the Woolworths crawler"""
    
    # Set up argument parser
    parser = argparse.ArgumentParser(description="Test Woolworths Crawler")
    ...
    parser.add_argument("--output-dir", default="./data/woolworths_test", help="Output directory")
    
    args = parser.parse_args()
    
    # Initialize crawler
    crawler = WoolworthsCrawler(
